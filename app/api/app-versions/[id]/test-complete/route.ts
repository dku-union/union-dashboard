...
export async function POST(
  ...
  try {
    ...
    if (!membership) {
      logger.warn("app_version.test_complete.denied", { requestId, actorId: session.id, versionId: id });
      return jsonError("버전 접근 권한이 없습니다.", 403, requestId, "VERSION_ACCESS_DENIED");
    }
    if (!canWriteAppVersion(membership.role)) {
      logger.warn("app_version.test_complete.denied", {
        requestId,
        actorId: session.id,
        versionId: id,
        role: membership.role,
      });
      return jsonError("테스트 완료 처리 권한이 없습니다.", 403, requestId, "VERSION_WRITE_DENIED");
    }
    ...
    if ("error" in result) {
      logger.warn("app_version.test_complete.spring_failed", {
        requestId,
        actorId: session.id,
        versionId: id,
    ...
    logger.info("app_version.test_complete.succeeded", { requestId, actorId: session.id, versionId: id });
    return jsonData(result.data, requestId);
  } catch (error) {
    return serverError("app_version.test_complete.failed", error, requestId, undefined, {
      actorId: session.id,
      versionId: id,
    });
  }
